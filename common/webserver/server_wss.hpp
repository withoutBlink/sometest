...
namespace SimpleWeb {
  ...
  public:
    SocketServer(const std::string &cert_content, const std::string &private_key_content)
        : SocketServerBase<WSS>(443), context(asio::ssl::context::tlsv12) {
      context.use_certificate_chain(asio::buffer(cert_content));
      context.use_private_key(asio::buffer(private_key_content), asio::ssl::context::pem);
    }
